...
const Navy = {
  currentLevel: 0,
  level: [] as any[],
  drawer: undefined as Element | undefined,
  overlay: undefined as Element | undefined,
  nav: undefined as Element | undefined,
  container: undefined as HTMLElement | undefined,
  currentRelatedMenu: undefined as HTMLElement | undefined,
  currentMenuLink: undefined as HTMLElement | undefined,
  currentBtn: undefined as HTMLElement | undefined,
  currentMenuBtn: undefined as HTMLElement | undefined,
  ...
  resizeDrawerHeight(target: Element | undefined) {
    if (!target) {
      return
    }
    const paddingTop = Navy.drawer ? parseInt(
      window
        .getComputedStyle(Navy.drawer, null)
        .getPropertyValue('padding-top'),
    ) : 0

    const paddingBottom = Navy.drawer ? parseInt(
      window
        .getComputedStyle(Navy.drawer, null)
        .getPropertyValue('padding-bottom'),
    ) : 0
    const newHeight = paddingTop + paddingBottom + (target as HTMLElement).offsetHeight + 'px';
    if (Navy.drawer) {
      (Navy.drawer as HTMLElement).style.height = newHeight
    }
  },

  displayRelatedSubmenu(btn: HTMLElement, relatedMenu: HTMLElement | undefined, submenus: NodeListOf<Element>, isBackBtn?: boolean) {
    if (relatedMenu === undefined) {
      return
    }
    ;[].forEach.call(submenus, function (submenu: HTMLElement) {
  ...
  parseTree(ul: HTMLElement, level: number) {
    ...
    const submenus = ul.querySelectorAll(':scope > li > ul')
      ...
      ;[].forEach.call(nextButtons, function (btn: unknown) {
        (btn as unknown as any).relatedMenu = (btn as unknown as any).nextElementSibling

        // inject menu in it's respective slide:
        Navy.level[level + 1].appendChild((btn as unknown as any).relatedMenu)

          (btn as unknown as any).addEventListener('click', function () {
            Navy.showLevel(level + 1)

            Navy.displayRelatedSubmenu(btn as HTMLElement, (btn as unknown as any).relatedMenu, submenus)
            Navy.currentMenuLink = btn as HTMLElement
          })

        // recursion for next navigation levels
        Navy.parseTree((btn as unknown as any).relatedMenu, level + 1)
      })
      ;[].forEach.call(backButtons, function (backBtn: any) {
        backBtn.relatedMenu =
  ...
  initMobile(navigationItems: any, target: any) {
  ...
  setDrawerXPosition(btn: any) {
    ...
    (Navy.drawer as any).style.transform = `translateX(${leftPos}px)`

    Navy.currentBtn = btn
  },

  closeSubmenu(mainmenuBtn: any) {
  ...
  toggleSubmenu(mainmenuBtn: any, relatedMenu: any, submenus: any) {
  ...
  initDesktop(options: any) {
    ...
    if (options.overlay) {
      Navy.overlay = document.querySelector(options.overlay)
    }
    ...
    window.addEventListener('resize', function () {
      ...
      ;[].forEach.call(mainmenuBtns, function (mainmenuBtn: any) {
        ...
        // add click events
        mainmenuBtn.addEventListener('click', function (event: any) {
